...
using FinancialModeling.Providers;
using FinancialModeling.Results;
...
namespace FinancialModeling.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        [AllowAnonymous]
        [Route("Register")]
        public async Task<RegisterBindingModel> Register(RegisterBindingModel model)
        {
            model.IsSucess = false;
            if (!ModelState.IsValid)
                model.Message = "Please fill all the fields properly.";

            IdentityResult result = null;
            var user = new ApplicationUser() { UserName = model.Email, Email = model.Email, ClientId = model.ClientId, IsActive = true };
            user.CreatedBy = User.Identity.GetUserId();
            user.CreatedDate = DateTime.UtcNow;
            try
            {
                result = await UserManager.CreateAsync(user, model.Password);
                model.Message = "Registration Sucessfully.";

                //Add user to selected Role
                if (result.Succeeded)
                    UserManager.AddToRole(user.Id, model.UserRole.RoleName);
            }
            catch (Exception ex)
            {
            ...
            model.IsSucess = result.Succeeded;
            return model;
        }
